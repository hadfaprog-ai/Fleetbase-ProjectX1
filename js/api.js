...
// ── Error Logger (browser console + server log file)
const WIM_LOGGER = {
  _enabled: true,
...
const WIM_API = {
  ...
  init(config) {
    ...
    this.token = config?.token || WIM_CONFIG.api.token;
    this.driverId = config?.driverId || WIM_CONFIG.api.driverId;
    this.companyId = config?.companyId || WIM_CONFIG.api.companyId;
  ...
  async getGeofenceEvents(params = '') {
    try {
      ...
      // Handle paginated response: { current_page: 1, data: [...], total: 0 }
